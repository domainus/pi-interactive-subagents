...
function confirm(ctx: AnyCtx, message: string, signal?: AbortSignal): Promise<boolean> { return ctx.ui.confirm("Workflow confirmation", message, { signal }); }
...
export function registerWorkflowUI(pi: ExtensionAPI, options: { owner: symbol; launcher: (parent: any, owner: symbol, signal: AbortSignal) => WorkflowNodeLauncher; moduleSignal: AbortSignal }): { shutdown(): Promise<void> } {
  let host: WorkflowHost | undefined;
  let lastCtx: AnyCtx | undefined;
  const getHost = (ctx: AnyCtx) => {
    lastCtx = ctx;
    if (!host) host = createWorkflowHost({ parent: { sessionId: ctx.sessionManager.getSessionId(), cwd: ctx.cwd, sessionDir: ctx.sessionManager.getSessionDir() }, launcher: options.launcher(parentOf(ctx), options.owner, options.moduleSignal), modelRegistry: ctx.modelRegistry, owner: options.owner });
    return host;
  };
  ...
  const start = (ctx: AnyCtx, id: string, confirmBuild: boolean, resumeRun = false): Result => {
    const activeHost = getHost(ctx); const execution = resumeRun ? activeHost.resume(id, confirmBuild) : activeHost.run(id, confirmBuild); const runningSnapshot = activeHost.statusSnapshot(id); setWidget(ctx, runningSnapshot);
  ...
  const plan = async (ctx: AnyCtx, p: any, signal?: AbortSignal): Promise<Result> => {
    const build = p.template === "build";
    if (build) { const accepted = await confirm(ctx, `Plan mutating build workflow ${p.workflowId}?`, signal); if (!accepted || signal?.aborted) return errorResult(new Error("confirmation rejected")); }
  ...
  const resume = async (ctx: AnyCtx, id: string, signal?: AbortSignal): Promise<Result> => { const snapshot = getHost(ctx).statusSnapshot(id); if (snapshot.metadata.template === "build") { const accepted = await confirm(ctx, `Resume mutating build workflow ${id}?`, signal); if (!accepted || signal?.aborted) return errorResult(new Error("confirmation rejected")); } return start(ctx, id, snapshot.metadata.template === "build", true); };
  const approve = async (ctx: AnyCtx, p: any, signal?: AbortSignal): Promise<Result> => { const preview = getHost(ctx).previewApproval(p.workflowId, p.nodeId, p.attempt); const message = `Approve ${preview.workflowId}/${preview.nodeId} attempt ${preview.attempt}? Evidence ${preview.evidenceDigest}. Changed: ${boundedFiles(preview.changedFiles, preview.changedFileCount)}`; const accepted = await confirm(ctx, message, signal); if (!accepted || signal?.aborted) return errorResult(new Error("confirmation rejected")); const value = getHost(ctx).approve(p.workflowId, p.nodeId, p.attempt, true); return textResult(`Approved ${p.workflowId}/${p.nodeId} attempt ${p.attempt}. Apply with workflow_apply using token ${value.token} (evidence ${value.evidenceDigest}).`, { ...value }); };
  const apply = async (ctx: AnyCtx, p: any, signal?: AbortSignal): Promise<Result> => { const preview = getHost(ctx).previewApply(p.workflowId, p.nodeId, p.token); const accepted = await confirm(ctx, `Apply token ${preview.token} for ${preview.workflowId}/${preview.nodeId} attempt ${preview.attempt} (evidence ${preview.evidenceDigest})?`, signal); if (!accepted || signal?.aborted) return errorResult(new Error("confirmation rejected")); const value = getHost(ctx).apply(p.workflowId, p.nodeId, p.token, true); return textResult(`Applied ${p.workflowId}/${p.nodeId} attempt ${value.attempt}.`, { workflowId: value.workflowId, nodeId: value.nodeId, attempt: value.attempt, applied: true }); };
  ...
  tool("workflow_plan", planParams, (p, c, signal) => plan(c, p, signal)); tool("workflow_run", idParams, (p, c, signal) => { const build = getHost(c).statusSnapshot(p.workflowId).metadata.template === "build"; return build ? confirm(c, `Run mutating build workflow ${p.workflowId}?`, signal).then(ok => ok && !signal?.aborted ? start(c, p.workflowId, true) : errorResult(new Error("confirmation rejected"))) : start(c, p.workflowId, false); }); tool("workflow_status", idParams, (p, c) => status(c, p.workflowId)); tool("workflow_cancel", idParams, (p, c, signal) => cancel(c, p.workflowId, signal)); tool("workflow_resume", idParams, (p, c, signal) => resume(c, p.workflowId, signal)); tool("workflow_approve", nodeParams, approve); tool("workflow_apply", applyParams, apply);
  ...
  command("workflow-run", (c, p) => { const id = p.workflowId; return getHost(c).statusSnapshot(id).metadata.template === "build" ? confirm(c, `Run mutating build workflow ${id}?`).then(ok => ok ? start(c, id, true) : errorResult(new Error("confirmation rejected"))) : start(c, id, false); }, true); command("workflow-status", (c, p) => status(c, p.workflowId), true); command("workflow-cancel", (c, p) => cancel(c, p.workflowId), true); command("workflow-resume", (c, p) => resume(c, p.workflowId), true); command("workflow-approve", approve); command("workflow-apply", apply);
