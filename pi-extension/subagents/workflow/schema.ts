...
export const GateResultSchema = Type.Object({ version: Type.Literal(WORKFLOW_VERSION), workflowId: Id, nodeId: Id, kind: GateKindSchema, passed: Type.Boolean(), checkedAt: Type.Integer({ minimum: 0 }), sourceNodeId: Type.Optional(Id), attempt: Type.Optional(Type.Integer({ minimum: 1, maximum: 100 })), evidenceDigest: Type.Optional(Digest), rawEnvelopeDigest: Type.Optional(Digest), evaluationId: Digest, gateDigest: Digest, error: Type.Optional(Type.String({ minLength: 1, maxLength: 4096 })) }, { additionalProperties: false });
const WorktreeEvidenceFields = { version: Type.Literal(WORKFLOW_VERSION), workflowId: Id, nodeId: Id, attempt: Type.Integer({ minimum: 1, maximum: 100 }), mode: Type.Union([Type.Literal("read-only"), Type.Literal("mutating")]), cwd: AbsolutePath, path: Type.Optional(AbsolutePath), base: Type.String({ pattern: "^[A-Fa-f0-9]{40,64}$" }), head: Type.String({ pattern: "^[A-Fa-f0-9]{40,64}$" }), diffHash: Digest, changedFiles: Type.Array(RelativePath, { maxItems: 512, uniqueItems: true }), evidenceDigest: Digest, capturedAt: Type.Integer({ minimum: 0 }), preserved: Type.Boolean() } as const;
...
function validateAttemptHistory(list: readonly NodeAttempt[]): boolean {
  ...
  for (let i = 0; i < list.length; i++) {
    const item = list[i]; if (item.attempt !== i + 1 || item.status === "pending" || (i < list.length - 1 && item.status !== "retrying" && item.status !== "cancelled")) return false;
    if (i > 0) {
...
export function validateWorkflowState(value: unknown, ownership?: { workflowId?: string; sessionId?: string }): ValidationResult<WorkflowState> {
  ...
  if (ownership?.workflowId && state.workflowId !== ownership.workflowId) issues.push(issue("/workflowId", "artifact ownership mismatch")); if (ownership?.sessionId && state.sessionId !== ownership.sessionId) issues.push(issue("/sessionId", "artifact ownership mismatch"));
